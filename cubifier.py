...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        Tk().withdraw()
        filename = askopenfilename(title = "Select G-Code",filetypes = (("G-Code file","*.gcode"),("all files","*.*")))
        if not filename:
            exit(1)
        print_type = detect_file_type(filename)
        pf = print_type(debug=debug)
        result_file = pf.process(filename)
